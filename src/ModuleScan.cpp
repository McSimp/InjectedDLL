...
ModuleScan::ModuleScan(const std::string& moduleName)
{
    m_moduleHandle = GetModuleHandle(Util::Widen(moduleName).c_str());
...
void* ModuleScan::Scan(const MemorySignature& sigStruct)
{
...
void* ModuleScan::Scan(const char* sig, const char* mask, int sigLength)
{
